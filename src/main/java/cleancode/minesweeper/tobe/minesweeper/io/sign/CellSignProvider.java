...
public enum CellSignProvider implements CellSignProvidable {
    ...
    NUMBER(CellSnapshotStatus.NUMBER) {
        @Override
        public String provide(CellSnapshot cellSnapshot) {
            return String.valueOf(cellSnapshot.getNearbhLandMineCount());
        }
    },
    ...
    public static String findCellSignFrom(CellSnapshot snapshot) {
        CellSignProvider cellSignProvider = findCellSignProvider(snapshot);
        return cellSignProvider.provide(snapshot);
    }

    private static CellSignProvider findCellSignProvider(CellSnapshot snapshot) {
